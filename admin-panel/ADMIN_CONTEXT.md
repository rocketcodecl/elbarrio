...
- El panel de galería está publicado en `https://admin.elbarrio.lat/`; el HTML, `index-5l2QfTIY.js` e `index-BlV3SA3u.css` públicos coinciden byte por byte con `admin-panel/dist` y el dominio responde HTTP 200. La versión previa quedó respaldada localmente en `admin-panel/admin-backups/20260812-ad-gallery-predeploy` y en el servidor como `index.pre-gallery-safe-20260812.html`.
