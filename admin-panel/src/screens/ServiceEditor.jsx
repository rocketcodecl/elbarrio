...
export default function ServiceEditor({
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        if (data?.length) {
          setRubros(data.map((item) => ({ key: item.key, icon: item.icon, label: item.name })));
        }
      });
  }, []);
  ...
  useEffect(() => {
    ...
    let request = supabase
      .from("profiles")
      .select(
        "id, full_name, avatar_url, verified, verification_status, neighborhood_id"
      )
      .order("full_name")
      .limit(500);
    ...
    request.then(({ data, error: providerError }) => {
      if (providerError)
        setError(
          `No fue posible cargar los prestadores: ${providerError.message}`
        );
      const rows = data || [];
      setProviders(rows);
      setDraft((current) => ({
        ...current,
        authorId: current.authorId || rows[0]?.id || profile?.id || "",
      }));
      setLoadingProviders(false);
    });
  ...
  const uploadImage = async (event) => {
    ...
    if (!file.type.startsWith("image/"))
      return setError("Selecciona una imagen válida.");
    if (file.size > 5 * 1024 * 1024)
      return setError("La imagen debe pesar menos de 5 MB.");
    setUploading(true);
    setError("");
    try {
  ...
  const save = async (event) => {
    ...
    if (!draft.authorId || !draft.title.trim() || !draft.content.trim())
      return setError(
        "Selecciona un prestador y completa título y descripción."
      );
    const selectedProvider = providers.find(
      (provider) => provider.id === draft.authorId
    );
    const targetNeighborhoodId =
      service?.neighborhood_id || selectedProvider?.neighborhood_id;
    if (!targetNeighborhoodId)
      return setError("El prestador seleccionado no tiene un barrio asignado.");
    ...
    const payload = {
      ...
      price,
      service_phone: draft.phone.trim() || null,
      service_whatsapp: draft.whatsapp.trim() || null,
      service_instagram: draft.instagram.trim().replace(/^@/, "") || null,
    };
    if (!editing || imageChanged)
      payload.images = draft.image ? [draft.image] : null;
    ...
    if (saveError)
      return setError(
        `No fue posible ${editing ? "guardar" : "crear"} el servicio: ${
          saveError.message
        }`
      );
    onSaved(service?.id);
  ...
  return (
    <div className="service-editor-page">
      <header className="subpage-header">
        <button className="subpage-back" type="button" onClick={onBack}>
          ←
        </button>
        <div>
          ...
          <span>
            {editing
              ? "Actualiza la información que aparece en la aplicación."
              : "Crea una publicación en nombre de un prestador del barrio."}
          </span>
        </div>
        <button
          className="button button-primary"
          type="submit"
          form="admin-service-form"
      ...
      >
        <section className="editor-section">
          <div className="editor-section-title">
            <span>1</span>
            <div>
              <h2>Prestador e imagen</h2>
              <p>El servicio quedará asociado al perfil seleccionado.</p>
            </div>
          </div>
          <div className="admin-service-identity">
                ...
                {loadingProviders ? (
                  ...
                  providers.map((provider) => (
                    <option key={provider.id} value={provider.id}>
                      {provider.full_name || "Vecino sin nombre"}
                      {provider.verified ||
            ...
            <label className="admin-service-image">
              {draft.image ? (
                <img src={draft.image} alt="" />
              ) : (
                <span>
                  🧰<strong>Imagen del servicio</strong>
                </span>
              )}
        ...
        <section className="editor-section">
          <div className="editor-section-title">
            ...
            <div>
              ...
              <p>Estos datos aparecerán en la aplicación.</p>
            </div>
          </div>
          <div className="admin-form-grid">
            <label className="field field-full">
              ...
              <input
                ...
                maxLength="120"
                placeholder="Ej: Reparaciones eléctricas a domicilio"
                required
              />
            </label>
            <label className="field">
            ...
            <label className="field">
              Valor desde <small>Opcional</small>
              <input
                type="number"
                min="0"
                step="1"
            ...
            <label className="field field-full">
              ...
              <textarea
                ...
                maxLength="3000"
                placeholder="Describe el servicio, experiencia y cobertura…"
                required
              />
            </label>
            <label className="field">
        ...
        <footer className="commerce-editor-footer">
          <button
