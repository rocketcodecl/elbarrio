...
export default function EventManager({ profile }) {
  ...
  const filtered = useMemo(() => {
    ...
    return events.filter(event => {
      ...
      const matchesStatus = filter === 'all' || (filter === 'active' && event.status === 'active') || (filter === 'paused' && event.status !== 'active')
  ...
  const toggleStatus = async event => {
    ...
    setChangingId(null)
    if (updateError) return setError('No fue posible actualizar la visibilidad del evento.')
    ...
    <div className="event-manager event-list-page">
      ...
      <section className="commerce-directory">
        <header className="directory-toolbar">
          ...
          <div className="filter-row directory-filters">{[['all', 'Todos'], ['active', 'Publicados'], ['paused', 'Pausados']].map(([value, label]) => <button key={value} type="button" className={filter === value ? 'is-active' : ''} onClick={() => setFilter(value)}>{label}</button>)}</div>
