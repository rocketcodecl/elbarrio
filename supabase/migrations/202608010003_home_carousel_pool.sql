-- Amplía la piscina editorial a quince contenidos; Inicio mezcla y muestra cinco.
...
alter table public.posts add constraint posts_home_carousel_order_check
...
begin
  ...
  if v_admin.id is null then raise exception 'No tienes permisos administrativos.'; end if;
  if p_neighborhood_id is null then raise exception 'Selecciona un barrio.'; end if;
  ...
  if coalesce(cardinality(p_post_ids), 0) > 15 then
    raise exception 'La portada admite un máximo de quince contenidos seleccionados.';
  end if;
  ...
  where id = any(coalesce(p_post_ids, array[]::uuid[]))
    and neighborhood_id = p_neighborhood_id and status = 'active'
