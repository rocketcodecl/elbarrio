-- Publicidad en los feeds de Servicios y Comercios.
-- Ejecutar manualmente después de 202608120002_advertising_gallery.sql.
...
-- El RPC vigente sigue disponible para el panel publicado. Esta versión
-- delega su validación y persistencia y luego aplica las cuatro ubicaciones.
...
begin
  ...
  -- El RPC anterior realiza toda la validación de superadministrador,
  -- campos, fechas, imágenes, barrios y escritura auditada.
  v_campaign_id := public.admin_upsert_advertising_campaign(
    ...
    p_cta_url => p_cta_url,
    -- El RPC anterior solo admite las dos ubicaciones originales.
...
comment on function public.admin_upsert_advertising_campaign_v2(uuid, text, text, text, text, text, text[], text, text, text, text[], uuid[], timestamptz, timestamptz, text, integer, numeric, text, text)
  is 'Crea o actualiza campañas con ubicaciones en Inicio, Actividad, Servicios y Comercios.';
