...
- La app incorpora reporte y bloqueo de contenido, evaluación de tratos cerrados, actores oficiales, feed de Inicio diversificado por actualidad/interacción, eventos recurrentes/seguidos y recordatorios internos deduplicados. Los bloqueos se respetan en Inicio, Mercado, Servicios, Alertas y búsqueda. El buscador global queda técnicamente disponible pero oculto de la navegación hasta rediseñarlo.
- Perfil se mantiene deliberadamente breve: seis acciones principales. Editar perfil, notificaciones, Modo accesible, Privacidad, Nosotros y eliminación de cuenta viven dentro del engranaje. No volver a añadir accesos principales sin revisar antes la cantidad total del menú.
...
- Auditoría de scroll del 10 de agosto: solo `currentScreen='main'` puede mantener visible el TabBar. Esto evita que detalles como Eventos queden cubiertos por la navegación inferior. Todos los contenedores flexibles principales de Inicio, Mercado, Servicios, Comercios, Noticias, Chat, Alertas, Eventos y administración móvil declaran `minHeight: 0`; Eventos reserva además espacio real para el menú y su acción inferior.
...
- La app principal y el panel compilan; el lint focalizado pasa. El panel fue publicado en `https://admin.elbarrio.lat/`; `index-0s7QwIl5.js`, `index-n8Oe7OA7.css` y el HTML público coinciden byte por byte con el build local y el dominio responde HTTP 200. La versión anterior quedó respaldada localmente y en el servidor como `index.pre-advertising-20260812.html`. No se ha sincronizado Capacitor ni generado una nueva entrega móvil para este bloque.
- `202608120002_advertising_gallery.sql` fue ejecutada con resultado `Success`, según confirmación manual del usuario. La columna `image_urls` respondió HTTP 200 y la RPC actualizada rechazó correctamente una sesión anónima con HTTP 401.
- Cada campaña admite de una a tres gráficas ordenadas de proporción recomendada `1200 × 628`. La app muestra la gráfica a todo el ancho, sin título, descripción, anunciante ni CTA duplicados; toda su superficie abre el enlace y solo superpone la etiqueta mínima `Publicidad`. Las gráficas alternan cada 3,6 segundos con fundido y gesto lateral; una sola conserva el comportamiento estático. En Inicio aparece bajo “Para ti” y en Actividad después de la tercera publicación.
- El panel corregido fue publicado en `https://admin.elbarrio.lat/`; el HTML, `index-BNdDdGOv.js` e `index-DKClirSj.css` coinciden byte por byte con el build local y responden HTTP 200. Anunciante, nombre de campaña, monto, pago y notas son internos; el editor visible pide enlace y de una a tres gráficas.
- APK de prueba regenerado después de sincronizar Capacitor: `release/android/el-barrio-prueba-20260812/el-barrio-prueba-publicidad.apk`. Usa el paquete `lat.elbarrio.app`, versión `1.0.1` (`versionCode 2`), pesa aproximadamente 19 MB y su SHA-256 es `538f41a4ad52a4d90af4de5760c188fc42288d42e50af7f071cc54ed1511d1fa`. `apksigner` confirmó firma debug válida mediante APK Signature Scheme v2. Es solo para instalación y prueba directa; no sirve como entrega firmada para Google Play y no reemplaza el AAB 1.0.1 existente.

## Continuidad del arranque nativo — 12 de agosto de 2026
...
## Expansión publicitaria a Mercado, Eventos y mapa — 16 de agosto de 2026

- Punto de retorno local anterior al bloque: tag `pre-expansion-publicitaria-20260816` sobre `b10d6d0`.
- La auditoría confirmó que Comercios ya prioriza `is_premium` y Servicios programa `is_featured`; no se creó otro módulo ni se duplicó esa administración.
- El mapa ya consultaba `commerces.is_premium` pero no lo representaba. Ahora los comercios destacados usan un marcador diferenciado con estrella y su ficha inferior indica `Destacado`; los comercios normales e incidentes conservan su comportamiento.
...
- La migración preparada es `202608160001_advertising_marketplace_events.sql`. Amplía las restricciones y las RPC vigentes de cuatro a seis ubicaciones sin crear tablas ni un módulo adicional. **No está confirmada como ejecutada**; no asumir su aplicación ni usar `supabase db push`.
