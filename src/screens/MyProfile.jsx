...
function DetailSheet({ section, posts, favorites, deals, onClose, onNavigate }) {
  ...
  return (
    <div className="profile-sheet-backdrop" onClick={onClose}>
      <section className="profile-sheet" onClick={event => event.stopPropagation()}>
        <header>
          <div><small>Mi perfil</small><h2>{title}</h2></div>
          <button type="button" onClick={onClose} aria-label="Cerrar"><LineIcon name="close" /></button>
        </header>
            ...
            if (section === 'favorites') {
              return (
                <div className="profile-activity-row" key={item.id}>
                  <span>{item.cover_url || item.logo_url ? <img src={item.cover_url || item.logo_url} alt="" /> : '🏪'}</span>
                  <div><strong>{item.name}</strong><small>{item.category || 'Comercio del barrio'}</small></div>
                </div>
              )
            }
            return (
    ...
    <div className="my-profile">
      ...
      <div className="profile-scroll">
        <header className="profile-header">
          <h1>Mi perfil</h1>
          <button type="button" onClick={() => onNavigate?.('settings')} aria-label="Configuración"><LineIcon name="settings" size={21} /></button>
        </header>
...
.profile-badges{padding:17px 3px 15px}.profile-badges h3{font-size:14px;margin:0 0 11px}.profile-badges>div{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.profile-badges>div>span{display:flex;flex-direction:column;align-items:center;gap:6px}.profile-badges i{width:48px;height:48px;border-radius:14px;background:#e4f2ed;display:grid;place-items:center;font-style:normal;font-size:21px}.profile-badges span:nth-child(1) i{background:#fff7d7}.profile-badges span:nth-child(4) i{background:#f2e7fb}.profile-badges small{font-size:9.5px;font-weight:600}.profile-badges .is-locked{opacity:.38;filter:grayscale(.8)}
.profile-menu{background:#fff;border:1px solid #dce2de;border-radius:16px;overflow:hidden}.profile-menu-row{width:100%;min-height:64px;padding:9px 14px;display:flex;align-items:center;gap:12px;border-bottom:1px solid #e7ebe8;color:#1d211f;text-align:left}.profile-menu-row:last-child{border:0}.profile-menu-icon{width:36px;height:36px;border-radius:11px;display:grid;place-items:center;flex:0 0 auto;color:#159969}.profile-menu-copy{flex:1;min-width:0}.profile-menu-copy strong{display:block;font-size:13px;font-weight:700}.profile-menu-copy small{display:block;margin-top:2px;color:#7c858d;font-size:9px}.profile-menu-count{font-size:11px;color:#657078}.profile-menu-row>svg{color:#6f7a82}.profile-menu-row.is-danger{color:#ef3340}.profile-switch{width:38px;height:22px;padding:3px;border-radius:999px;background:#cfd6d2;transition:background .2s}.profile-switch i{display:block;width:16px;height:16px;border-radius:50%;background:#fff;transition:transform .2s;box-shadow:0 1px 3px rgba(0,0,0,.2)}.profile-switch.is-on{background:#159969}.profile-switch.is-on i{transform:translateX(16px)}.profile-bottom-space{height:18px}
.profile-sheet-backdrop{position:absolute;inset:0;z-index:120;background:rgba(18,25,21,.38);display:flex;align-items:flex-end}.profile-sheet{width:100%;max-height:78%;background:#f7faf7;border-radius:24px 24px 0 0;padding-bottom:24px;box-shadow:0 -10px 40px rgba(0,0,0,.14);animation:profileSheetIn .22s ease-out}.profile-sheet header{height:76px;padding:16px 18px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #e1e7e3}.profile-sheet header small{display:block;color:#159969;font-size:10px;font-weight:700}.profile-sheet header h2{margin:2px 0 0;font-size:18px}.profile-sheet header button{width:38px;height:38px;border-radius:50%;display:grid;place-items:center;background:#fff;border:1px solid #dce2de}.profile-sheet-list{max-height:calc(78vh - 76px);overflow:auto;padding:10px 14px}.profile-activity-row{width:100%;min-height:66px;display:flex;align-items:center;gap:11px;padding:8px;border-bottom:1px solid #e1e7e3;text-align:left;color:#1d211f}.profile-activity-row>span:first-child{width:48px;height:48px;border-radius:12px;overflow:hidden;background:#e5f5ef;display:grid;place-items:center;flex:0 0 auto}.profile-activity-row img{width:100%;height:100%;object-fit:cover}.profile-activity-row div{flex:1;min-width:0}.profile-activity-row strong,.profile-activity-row small{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.profile-activity-row strong{font-size:12px}.profile-activity-row small{margin-top:4px;color:#68727b;font-size:9.5px}.profile-sheet-empty{padding:42px 18px;text-align:center;color:#68727b;font-size:12px}
@keyframes profileSheetIn{from{transform:translateY(35px);opacity:.6}to{transform:translateY(0);opacity:1}}
